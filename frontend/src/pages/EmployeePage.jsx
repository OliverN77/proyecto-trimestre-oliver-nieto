...
function ReservaKanbanCard({ reserva, actions, actualizando, onActualizar, catalogProductos, onPlatosActualizados, token }) {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.2rem", marginBottom: "0.625rem" }}>
                ...
                {reserva.productos?.length > 0 && (
                    <div style={{ marginTop: "0.3rem" }}>
                        <p style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--oliva)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "0.1rem" }}>🍽️ Platos</p>
                ...
                {reserva.servicios?.length > 0 && (
                    <div style={{ marginTop: "0.3rem" }}>
                        <p style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--notte)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "0.1rem" }}>✨ Servicios</p>
                        <p style={{ fontSize: "0.75rem", color: "rgba(36,27,18,0.6)", margin: 0 }}>
                ...
                <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                    {actions.map((action) => (
                        <button
                            ...
                            style={{ padding: "0.3rem 0.7rem", borderRadius: "0.45rem", fontSize: "0.72rem", fontWeight: 600, border: "none", cursor: isUpdating ? "not-allowed" : "pointer", opacity: isUpdating ? 0.6 : 1, transition: "opacity 150ms ease", ...action.style }}
                        >
...
function ReservationsBoard({ token, catalogProductos }) {
    ...
    return (
        <div>
            {/* Filter bar */}
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", alignItems: "flex-end", marginBottom: "1.5rem", background: "white", borderRadius: "1rem", padding: "1rem 1.25rem", boxShadow: "0 1px 6px rgba(0,0,0,0.07)" }}>
    ...
    useEffect(() => {
        ...
        fetch(`${API_URL}/productos`, { headers: { Authorization: `Bearer ${token}` } })
            ...
            .then(({ res, data }) => {
                ...
                else setError("No se pudo cargar el catálogo de productos")
            })
